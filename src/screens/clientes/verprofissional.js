...
const VerProfissional = ({ navigation, route }) => {
  ...
  return (
    <View style={seeprofessionalStyles.container}>
      ...
      <ImageBackground source={{ uri: profissional?.foto ? `http:///194.210.105.253:4041/cliente/${profissional?.foto}` : `http:///194.210.105.253:4041/cliente/uploads/image.jpg  ` }} style={seeprofessionalStyles.backgroundImage}></ImageBackground>
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={seeprofessionalStyles.item}>
                <View style={{ flexDirection: 'row', gap: 3 }}>
                  <Image source={{ uri: item.imagem ? `http:///194.210.105.253:4041/cliente/${item.imagem}` : `http:///194.210.105.253:4041/cliente/uploads/image.jpg  ` }} style={{ width: 100, height: 100, borderTopLeftRadius: 10, borderBottomLeftRadius: 10 }} />
                  <View>
                    <Text>{item.nome}</Text>
